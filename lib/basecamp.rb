...
class BasecampAPI
  ...
  class Attachment
	...
	def save
	  response = BasecampAPI.connection.post('/upload', content, 'Content-Type' => 'application/octet-stream')
  ...
  private
	...
	def request(path, parameters = {})
	  p convert_body(parameters)
	  response = BasecampAPI.connection.post(path, convert_body(parameters), "Content-Type" => content_type)
